...
- **作業中のホワイトノイズ** — 画面を開いている間のみ再生（Web Audio）

> **通知について**  
> プッシュ通知・バックグラウンド通知は提供していません。タイマーは **Web 画面完結** です。  
> バックグラウンドから戻った際は壁時計ベースでタイマーを自動 catch-up します。
...
| **4.0 hPa 以上** | reduced | 25分 / 5分 | 6 | **1回** | 1回完了でグレーアウト「本日の学習は終了しました」 |

- 作業フェーズ中のみホワイトノイズを再生
- 1日の使用回数は localStorage で管理（日付が変わるとリセット）
- 実行中のタイマー状態も localStorage に保存（再読み込み・復帰時に catch-up）
