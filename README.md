...
| 条件 | 結果 |
|------|------|
| 直近7日間（上位・下位1日を除く）平均睡眠 **≥ 7h** **かつ** 昨日の歩数 **≥** 過去7日間（上位・下位1日を除く）平均の **70%** | 体調良好 → ポモドーロ有効 |
| 直近7日間（上位・下位1日を除く）平均睡眠 **≤ 7h** | **睡眠日** — タイマー無効・画面グレーアウト |
| 昨日の歩数 **<** 過去7日間（上位・下位1日を除く）平均の **70%** | **運動日** — タイマー無効・画面グレーアウト |
| 睡眠・歩数データ未取得 | **データ取得中** — タイマー無効 |

- **平均睡眠**: 直近7日間から、睡眠時間が最も長い1日と最も短い1日を除いた5日間の平均（記録のない日は0時間として扱う）
- **昨日の歩数**: カレンダー上の前日の歩数
